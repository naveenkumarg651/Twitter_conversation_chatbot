...
with open("chatbot_dataset.txt",encoding='utf8') as f:
    for line in f:
#        if i>1500:
#            break
#        i+=1
        que,ans=line.rstrip().split('\t')
        questions.append(que)
        a='<sos> '+ans
        b=ans+' <eos>'
        answers_input.append(a)
        answers_output.append(b)

tokenizer=Tokenizer(num_words=20000)
tokenizer.fit_on_texts(questions)
encoder_data=tokenizer.texts_to_sequences(questions)
encoder_word2idx=tokenizer.word_index
encoder_length=max(len(s) for s in encoder_data)
encoder_vocab_size=len(encoder_word2idx)+1


tokenizer=Tokenizer(num_words=20000,filters='')
tokenizer.fit_on_texts(answers_input+answers_output)
decoder_input_data=tokenizer.texts_to_sequences(answers_input)
decoder_output_data=tokenizer.texts_to_sequences(answers_output)
decoder_length=max(len(s) for s in decoder_input_data)
...
word2vec = {}
with open('wordvecs100.txt',encoding='utf8') as f:
...
r=model.fit([encoder_data,decoder_input_data,np.zeros((len(decoder_input_data),latent_dim),dtype="float32"),np.zeros((len(decoder_input_data),latent_dim),dtype="float32")],decoder_targets,validation_split=0.2,batch_size=64,epochs=100)
...
for i in range(10):
    j=np.random.randint(1,len(encoder_data))
    print(questions[j])
    s=encoder_data[j:j+1]
    Answer = decode_sequence(s)
    print(Answer)
    print("\n")
